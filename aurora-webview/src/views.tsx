...
@injectable()
export class PolylineArrowDraftEdgeView extends PolylineEdgeView {
    ...
    protected override renderLine(edge: SEdgeImpl, segments: Point[], context: RenderingContext): VNode {
        ...
        return <path d={path} class-sprotty-draft-edge={true} />;
    }
    ...
    protected override renderAdditionals(edge: SEdgeImpl, segments: Point[], context: RenderingContext): VNode[] {
        ...
        const p2 = segments[segments.length - 1];
        return [
            <path class-sprotty-draft-edge-arrow={true} d='M 6,-3 L 0,0 L 6,3 Z'
                  transform={`rotate(${this.angle(p2, p1)} ${p2.x} ${p2.y}) translate(${p2.x} ${p2.y})`}/>
        ];
    }
...
@injectable()
export class PolylineArrowUrgentEdgeView extends PolylineEdgeView {
    ...
    protected override renderLine(edge: SEdgeImpl, segments: Point[], context: RenderingContext): VNode {
        ...
        return <path d={path} class-sprotty-urgent-edge={true} />;
    }
    ...
    protected override renderAdditionals(edge: SEdgeImpl, segments: Point[], context: RenderingContext): VNode[] {
        ...
        const p2 = segments[segments.length - 1];
        return [
            <path class-sprotty-urgent-edge-arrow={true} d='M 6,-3 L 0,0 L 6,3 Z'
                  transform={`rotate(${this.angle(p2, p1)} ${p2.x} ${p2.y}) translate(${p2.x} ${p2.y})`}/>
        ];
    }
